...
int rastBBox_bbox_check( int   v0_x,     //uPoly
			  ...
			  int  q,        //uPoly
			  int  valid_Poly,
			  int   ll_x,     //BBOX#include "rastBBox_fix_sv.h"
#include <stdio.h>
#include <limits.h>
#include <assert.h>
			  ...
			  )
{
  ...
  valid = (! ((ll_x > screen_w) || (ll_y > screen_h) || (ur_x < 0) || (ur_y < 0)) ) && valid_Poly && (((poly.v[1].x[0]-poly.v[0].x[0])*(poly.v[2].x[1]-poly.v[1].x[1]))-((poly.v[2].x[0]-poly.v[1].x[0])*(poly.v[1].x[1]-poly.v[0].x[1]))<0); 
